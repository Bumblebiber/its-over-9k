...
export function isAgentProcessCmdline(cmdline, cli, envMarker = "O9K_USAGE_COLLECT") {
  ...
  if (cli === "codex") {
    return /(?:^|\/)codex(?:\s|$)/.test(cmdline);
  }
  if (cli === "cursor") {
    return /(?:^|\/)cursor-agent(?:\s|$)/.test(cmdline);
  }
  return false;
}
 ...
 * @param {{ excludePids?: number[], envMarker?: string, listPids?: () => number[], readCmdline?: (pid: number) => string|null }} [opts]
 */
