...
// Statusline lives in the standalone o9k-statusline package now. All o9k
// still knows is how to spot wiring left behind by an older in-tree install.
test("doctor flags a statusLine still pointing at the removed in-tree script", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "o9k-orphan-"));
  const cursorCfg = path.join(tmp, ".cursor/cli-config.json");
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      statusLine: {
        type: "command",
        command: "/usr/bin/node /m/plugins/o9k-core/scripts/statusline/o9k-statusline.mjs --host cursor",
      },
    })
  );
  ...
  assert.equal(artifact?.state, "orphaned");
  assert.ok(r.problems.some((p) => p.includes("o9k-statusline")));

  fs.rmSync(tmp, { recursive: true, force: true });
});

test("doctor stays quiet about a foreign statusLine it never owned", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "o9k-foreign-"));
  const cursorCfg = path.join(tmp, ".cursor/cli-config.json");
  fs.mkdirSync(path.dirname(cursorCfg), { recursive: true });
  fs.writeFileSync(cursorCfg, JSON.stringify({ statusLine: { command: "echo foreign" } }));
