...
#
# usage: run-bench.sh <combo-name> <sandbox-config-dir> [model]
#
...
#
# Requires: claude, jq, git, node. Costs real API/plan tokens.
set -euo pipefail
...
echo "combo=$COMBO model=$MODEL tasks_hash=$TASKS_HASH target=$TARGET_REF"
...
for task_dir in "$BENCH_DIR"/tasks/t*/; do
  ...
  out="$(cd "$tree" && CLAUDE_CONFIG_DIR="$SANDBOX" claude -p "$(cat "$task_dir/prompt.md")" \
    --model "$MODEL" --output-format json --max-turns 40 --permission-mode acceptEdits 2>"$WORK/$task.err")"
  claude_rc=$?
  set -e

  if [ $claude_rc -ne 0 ] || [ -z "$out" ]; then
    echo "   claude run failed (rc=$claude_rc), see $WORK/$task.err"
    row="$(jq -n --arg t "$task" '{task:$t, pass:false, error:"claude_run_failed"}')"
  else
    set +e
    BENCH_TASK_DIR="$task_dir" bash "$task_dir/check.sh" "$tree" >/dev/null 2>&1
    pass=$([ $? -eq 0 ] && echo true || echo false)
    set -e
    row="$(printf '%s' "$out" | jq --arg t "$task" --argjson p "$pass" '{
      task: $t, pass: $p,
